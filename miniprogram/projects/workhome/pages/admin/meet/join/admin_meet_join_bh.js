	...
	methods: {
			...
			try {
				...
				await cloudHelper.callCloudSumbit(this.data.oprt + '/meet_cancel_time_join', params, opt).then(res => {
					let callback = () => {
						...
						wx.redirectTo({
							url: `${this.data.oprt}_meet_join?meetId=${this.data.meetId}&mark=${this.data.mark}&title=${this.data.titleEn}&time=${this.data.time}&dayidx=${this.data.parentDayIdx}&timeidx=${this.data.parentTimeIdx}`,
						});
					}
		...
		bindStatusTap: async function (e) {
			...
			let callback = async () => {
				...
				try {
					await cloudHelper.callCloudSumbit(this.data.oprt + '/join_status', params, opts).then(res => {
